...
def get_inference_services(namespace):
    """Return a list of InferenceService and DynamoGraphDeployment CRs as json objects."""
    # Fetch KServe InferenceServices
    gvk_kserve = versions.inference_service_gvk()
    try:
        kserve_list = api.list_custom_rsrc(**gvk_kserve, namespace=namespace)["items"]
    except Exception as e:
        log.warning("Failed to list InferenceServices: %s", e)
        kserve_list = []

    # Fetch DynamoGraphDeployments
    gvk_dynamo = versions.dynamo_graph_deployment_gvk()
    try:
        dynamo_list = api.list_custom_rsrc(**gvk_dynamo, namespace=namespace)["items"]
    except Exception as e:
        log.warning("Failed to list DynamoGraphDeployments: %s", e)
        dynamo_list = []

    # Combine lists
    items = kserve_list + dynamo_list

    return api.success_response("inferenceServices", items)


@bp.route("/api/namespaces/<namespace>/inferenceservices/<name>")
def get_inference_service(namespace, name):
    """Return an InferenceService or DynamoGraphDeployment CR as a json object."""

    # Try fetching as InferenceService
    try:
        ...
        # If API fails for other reasons, assume not found or try next
        inference_service = None

    # If not found, try fetching as DynamoGraphDeployment
    if not inference_service:
        try:
            inference_service = api.get_custom_rsrc(
                **versions.dynamo_graph_deployment_gvk(), namespace=namespace, name=name
            )
        except Exception as e:
            # If both fail, raise the error (likely NotFound)
            log.error("Failed to find resource %s in %s: %s", name, namespace, e)
            raise NotFound("InferenceService or DynamoGraphDeployment not found.")
