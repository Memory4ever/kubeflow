...
export class IndexComponent implements OnInit, OnDestroy {
  ...
  private parseInferenceService(inferenceService: InferenceServiceIR) {
    ...
    if (inferenceService.kind === 'DynamoGraphDeployment') {
         inferenceService.ui.predictorType = 'Dynamo Graph' as any;
         // Use spec.services keys as component summary if possible, or just default text
         // Dynamo CRD doesn't have standard KServe predictor/runtime fields
         inferenceService.ui.runtimeVersion = 'vLLM Runtime';
         inferenceService.ui.storageUri = 'N/A';
         inferenceService.ui.protocolVersion = 'v1';
         inferenceService.ui.link = {
           text: inferenceService.metadata.name,
           // Currently redirecting to details page might show empty info because details page also expects KServe structure
           // But at least the list won't crash.
