...
export function getK8sObjectUiStatus(obj: K8sObject): Status {
  ...
  if (condition === undefined) {
    // Basic status check if conditions are missing (e.g. Dynamo might just have a phase)
    if (obj.status.phase === 'Ready' || obj.status.state === 'Ready') {
...
export function getPredictorExtensionSpec(
  ...
  if (!predictor) {
      return {} as PredictorExtensionSpec;
  }
...
export function parseRuntime(inferenceService: InferenceServiceK8s): string {
  if (!inferenceService.spec || !inferenceService.spec.predictor) {
      if (inferenceService.kind === 'DynamoGraphDeployment') {
          return 'Dynamo Runtime';
      }
      return '';
  }
...
export function getPredictorRuntime(predictor: PredictorSpec): string {
  if (predictor === null || predictor === undefined) {
    return '';
  }
